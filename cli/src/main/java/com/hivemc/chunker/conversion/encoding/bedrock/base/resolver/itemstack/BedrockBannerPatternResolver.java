...
/**
 * Banner Pattern resolver, used for Bedrock
 */
public class BedrockBannerPatternResolver implements Resolver<String, ChunkerBannerPattern> {
    private final InvertibleMap<ChunkerBannerPattern, String> mapping = InvertibleMap.enumKeys(ChunkerBannerPattern.class);

    /**
     * Create a new bedrock banner pattern resolver.
     *
     * @param version the game version being used, as certain banners are only available after specific versions.
     */
    public BedrockBannerPatternResolver(Version version) {
        ...
        if (version.isGreaterThanOrEqual(1, 21, 0)) {
            mapping.put(ChunkerBannerPattern.FLOW, "flw");
            mapping.put(ChunkerBannerPattern.GUSTER, "gus");
        }
    }
